  ...
  for (const entry of history) {
    ...
    if (entry.answerType === 'scale' && typeof entry.answer === 'string') {
      ...
      if (
        ...
        if (textLower.match(/(no |not |haven't|don't)/g)?.length > 1) {
...
export async function POST(request: Request) {
  try {
    ...
    const { action, currentPhaseName, history, industry, userName } = await request.json();
...
async function handleReportGeneration(history: ScorecardHistoryEntry[], industry: string, userName?: string) {
  try {
    ...
    if (userAITier === 'Dabbler') {
      ...
      keyFindingsInstructions = "Highlight basic strengths and fundamental weaknesses. Explain the impact of weaknesses in simple terms.";
...
FINAL REMINDER: DO NOT add ANY additional content after the Learning Path section. DO NOT include any promotions, advertisements, disclaimers, or external links to services like Homestyler, Wren AI, or other tools. The report MUST END with your Learning Path content.`;
    ...
    const requestBody = {
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Analyze the following assessment history for the ${industry} industry and generate the comprehensive Markdown report as instructed. IMPORTANT: Do NOT include any advertisements, promotional content, or external links in your response: ${JSON.stringify(history)}`
        }
      ],
      temperature: 0.7,
      max_tokens: 4000, // Set a specific token limit to avoid truncation
    };
    ...
    let reportMarkdown = completion.choices[0].message.content;
...
Return JSON: {
  ...
  "reasoning_text": string
    ...
    // --- Attempt Pollinations API ---
    try {
      console.log('>>> BACKEND: Attempting Pollinations API for question generation');
      const requestBody = {
        model: "openai-large", // Using openai-large as per existing code
        messages: [
      ...
      console.log('>>> BACKEND: Pollinations Question Generation API Request Body:', JSON.stringify(requestBody, null, 2));

      // Increase retries and delay for question generation calls
      const response = await fetchWithRetry(POLLINATIONS_API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(requestBody),
      }, 5, 5000); // Increased retries to 5, delay to 5000ms

      if (!response.ok) {
        const errorBody = await response.text();
        console.error('>>> BACKEND: Pollinations Question Generation API Error Response Body:', errorBody);
        throw new Error(`Pollinations API error: ${response.status}`);
      }

      const completion = await response.json();
      console.log('>>> BACKEND: RAW Pollinations Question Generation API Response:', JSON.stringify(completion, null, 2));

      parsedResponse = JSON.parse(completion.choices[0].message.content);
      console.log('>>> BACKEND: Parsed Pollinations Question Generation API Response:', JSON.stringify(parsedResponse, null, 2));

      // Check if the AI returned a question
      if (!parsedResponse.questionText) {
        console.error('>>> BACKEND: Pollinations AI prematurely generated a report or returned an invalid response instead of a question.');
        console.error('>>> BACKEND: Pollinations AI Response Content:', completion.choices[0].message.content);
        throw new Error('Pollinations AI returned invalid question format.');
      }

    } catch (pollinationsError: any) {
      console.error('>>> BACKEND: Pollinations API failed:', pollinationsError.message);
      usedApi = 'Groq'; // Attempt Groq as fallback

      // --- Attempt Groq API ---
      try {
        console.log('>>> BACKEND: Attempting Groq API for question generation');
        ...
        console.log('>>> BACKEND: Groq Question Generation API Request Body:', JSON.stringify(requestBody, null, 2));
        ...
        if (!response.ok) {
          const errorBody = await response.text();
          console.error('>>> BACKEND: Groq Question Generation API Error Response Body:', errorBody);
          throw new Error(`Groq API error: ${response.status}`);
        }

        const completion = await response.json();
        console.log('>>> BACKEND: RAW Groq Question Generation API Response:', JSON.stringify(completion, null, 2));

        parsedResponse = completion.choices[0].message.content ? JSON.parse(completion.choices[0].message.content) : null;
        console.log('>>> BACKEND: Parsed Groq Question Generation API Response:', JSON.stringify(parsedResponse, null, 2));

        // Check if the AI returned a question
        if (!parsedResponse || !parsedResponse.questionText) {
           console.error('>>> BACKEND: Groq AI returned invalid response or no questionText.');
           console.error('>>> BACKEND: Groq AI Response Content:', completion.choices[0].message.content);
           throw new Error('Groq AI returned invalid question format.');
        }

      } catch (groqError: any) {
        console.error('>>> BACKEND: Groq API failed:', groqError.message);
        usedApi = 'Gemini'; // Attempt Gemini as fallback

        // --- Attempt Gemini API ---
        try {
          console.log('>>> BACKEND: Attempting Gemini API for question generation');
          ...
          const requestBody = {
            contents: [
              { role: "user", parts: [{ text: systemPrompt + "\nBased on history: " + JSON.stringify(history) + ", generate next question. Return JSON only." }] }
            ],
            generationConfig: {
              temperature: 0.7,
          ...
          console.log('>>> BACKEND: Gemini Question Generation API Request Body:', JSON.stringify(requestBody, null, 2));
          ...
          if (!response.ok) {
            const errorBody = await response.text();
            console.error('>>> BACKEND: Gemini Question Generation API Error Response Body:', errorBody);
            throw new Error(`Gemini API error: ${response.status}`);
          }

          const completion = await response.json();
          console.log('>>> BACKEND: RAW Gemini Question Generation API Response:', JSON.stringify(completion, null, 2));

          // Gemini response structure is different
          parsedResponse = completion.candidates?.[0]?.content?.parts?.[0]?.text ? JSON.parse(completion.candidates[0].content.parts[0].text) : null;
          console.log('>>> BACKEND: Parsed Gemini Question Generation API Response:', JSON.stringify(parsedResponse, null, 2));

          // Check if the AI returned a question
          if (!parsedResponse || !parsedResponse.questionText) {
             console.error('>>> BACKEND: Gemini AI returned invalid response or no questionText.');
             console.error('>>> BACKEND: Gemini AI Response Content:', completion.candidates?.[0]?.content?.parts?.[0]?.text);
             throw new Error('Gemini AI returned invalid question format.');
          }

        } catch (geminiError: any) {
          console.error('>>> BACKEND: Gemini API failed:', geminiError.message);
          usedApi = 'None'; // All fallbacks failed
          throw new Error(`All AI APIs failed to generate a question. Pollinations: ${pollinationsError.message}. Groq: ${groqError.message}. Gemini: ${geminiError.message}`);
        }
      }
    }

    // If we reached here, one of the APIs successfully returned a parsed response with questionText

    // Check for repeated questions (using the successfully parsed response)
    if (history.some(qa => qa.question === parsedResponse.questionText)) {
      console.warn(`>>> BACKEND: AI (${usedApi}) generated a repeated question. Attempting to move to the next phase.`);
      // Move to next phase if question repeats
      if (currentPhaseIndex < ASSESSMENT_PHASES.length - 1) {
        const nextPhase = ASSESSMENT_PHASES[currentPhaseIndex + 1];
        return await handleAssessmentRequest(nextPhase, history, industry);
      } else {
        // If no more phases, consider assessment complete (should be caught by MAX_QUESTIONS check earlier, but as a safeguard)
         console.warn(`>>> BACKEND: AI (${usedApi}) generated a repeated question in the last phase. Marking assessment as completed.`);
         return NextResponse.json({
            questionText: null,
            answerType: null,
            options: null,
